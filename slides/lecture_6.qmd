...
## Application: Bootstrap Confidence Intervals {.smaller}

- When the population is normally distributed $N(\mu,\sigma^2)$, we used the t-statistic
$$
t = \frac{\overline{x} - \mu}{\ese}
$$
to form a confidence interval $[a,b]$ for the population mean $\mu$
$$
P(\mu \in [a,b]) = 0.95 \,, \quad 
a = \overline{x} - t^* \times \ese , \quad  
b = \overline{x} + t^* \times \ese
$$
where $t^* = t_{n-1}(0.025)$
...
- $\theta$ a population parameter $\,\,$ (e.g. $\, \theta =$ mean), $\,\, T$ an estimator for $\theta \,\,$ (e.g. $\, T =$ sample mean)
...
$$
These are the $\frac{\alpha}{2}100\%$ and $(1 − \frac{\alpha}{2})100\%$ percentiles of the sample distribution of $T^*_1, \ldots , T^*_B$
...
cat("t-test Confidence Interval (95%):", t.test.CI, "\n")
```
