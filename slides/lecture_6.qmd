...
**Simulating the confidence interval in R:**
...
- So far, our simulations used knowledge of the population distribution, e.g.
    ...
    * To simulate confidence intervals, we used that the population is $N(\mu,\sigma^2)$
    * To simulate the $\chi^2$ statistic, we sampled *Multinomial* counts


- **Problem:** If the population is unknown, where do we sample from?

- **Answer:** Sample from the sample itself!

The rub is we need to know μ to actually simulate the sampling distribution. For this particular question and others we will encounter in the upcoming chapters we will know the sampling distribution from theoretical means. So this issue will not be an impediment.
This section briefly touches on a computer-intensive means to work around the absence of a known sampling distribution, the bootstrap method [18], [16]. In its most basic application, we will see how the bootstrap method can be used to estimate the sampling distribution.
How? A basic assumption in simulation is that if we have enough ran- dom samples from a population, that the random samples can describe that population very well. This is the starting point for the bootstrap.
First a random sample x1, x2, . . . , xn is used to produce an empirical cumula- tive distribution function Fˆ(a) = #{i : xi ≤ a}/n. The assumption is that this ap- proximates well the actual cumulative distribution function F(a) = P(X ≤ a). As such, we assume we know a reasonable proxy for the population and use that as our model.

- **Problem:** What if the population distribution is unknown?

- **Solution:**
    * Suppose we have a sample available
    * The Bootstrap regards the sample as the **whole population**
    * The population distribution is estimated by **resampling**
    * This means samp(often with replacement) 
...
- A sample is drawn from a population $\qquad\qquad\qquad\qquad \qquad$ (Image from [Wikipedia](https://en.wikipedia.org/wiki/Bootstrapping_(statistics)#/media/File:Illustration_bootstrap.svg))

- From this sample, resamples are generated by drawing with replacement (orange)  

- Data points that were drawn more than once are shown in red and sligthly offsetted  
(approx. 26.4% of data points in the picture)  

- For each resample, the statistic $T$ is calculated 

- Therefore, a histogram can be calculated to estimate the distribution of $T$
...
## Why does the Bootstrap work? {.smaller}
...
## Example: Bootstrap Variance Estimation {.smaller}

::: {style="font-size: 0.91em"}

**Problem:** Suppose given a statistic $T$

- We want to estimate the variance of $T$ using bootstrap


**Example:** Consider the sample mean
$$
\overline{X} = \frac{1}{n} \sum_{i=1}^n X_i
$$

- If the population is $N(\mu,\sigma^2)$, we know that
$$
\Var[\overline{X}] = \frac{\sigma^2}{n}
$$

- If the population distribution is unknown, then 
$$
\Var[\overline{X}]  =  \text{ ???} \quad \implies \quad 
\text{Estimate } \, \Var[\overline{X}] \, \text{ via bootstrap}
$$
...
## Algorithm: Bootstrap Variance Estimation {.smaller}

Suppose given a sample $x_1, \ldots, x_n$

1. Draw sample $x_1^*,\ldots,x_n^*$ from $x_1, \ldots, x_n$ with replacement
    * In R, use ``sample(x, length(x), replace = TRUE)``
...
T^*_1, \ldots , T^*_B
$$
(These values are actually simulations from $T$)


## {.smaller}

4. The bootstrap estimate for the variance of $T$ is the sample variance of $T^*$
$$
\Var[T]_{\rm Boot} = \frac{1}{B-1} \sum_{i=1}^B  
\left(  T^*_i - \frac{1}{B}\sum_{j=1}^B T^*_j   \right)^2
$$
...
$$
\lim_{n \to \infty} \Var[T]_{\rm Boot} = 
\Var[T]
$$
...
## Example: Bootstrap for the mean {.smaller}

- For illustration purposes, suppose given a sample of size $n$ from $N(0,1)$

```r 
n <- # Enter sample size

# Generate illustrative random sample from N(0,1)
x <- rnorm(n)
```

- Since the population is $N(0,1)$, we know that
$$
\Var[\overline{X}] = \frac{\sigma^2}{n} = \frac{1}{n}
$$

- Now, we forget the sample came from $N(0,1)$

- We want to estimate $\Var[\overline{X}]$ using bootstrap
...
- One bootstrap simulation of $\overline{X}$ can be done as follows:
    1. Generate sample $x_1^*,\ldots,x_n^*$ from $x_1,\ldots,x_n$
    2. Compute the sample mean of $x_1^*,\ldots,x_n^*$
...
- To simulate $B = 10000$ bootstrap values from $\overline{X}$, use **replicate**

```r
# Generate B = 10000 bootstrap samples for xbar
B <- 10000;

xbar.star.vect <- replicate(B, {
                  x.star <- sample(x, n, replace = TRUE)
                  xbar.star <- mean(x.star)
                  })
```
...
- The vector ``xbar.star.vect`` contains $B = 10000$ bootstrap samples from $\overline{X}$

- Therefore, we think of ``xbar.star.vect`` as an approximation of $\overline{X}$

- The bootstrap variance is the sample variance of ``xbar.star.vect`` 

```r
# compute bootstrap variance of xbar
var.boot <- var(xbar.star.vect)

cat("The bootstrap estimate for the variance is", var.boot)
```

- We compare the bootstrap variance estimate to the *actual* variance 
$$
\Var[\overline{X}] = \frac{1}{n}
$$

```r
# compute the actual variance of xbar
var.true <- 1/n

cat("The bootstrap estimate for the variance is", var.boot)
cat("The theoretical value for the variance is", var.true)
cat("The error is", abs(var.boot - var.true))
```
...
- Let us estimate the variance with a sample size of $n = 10$

- The random sample ``x`` from which we bootstrap is

```{r}
#| echo: true

set.seed(23)    # Set seed to always get same randomization
...
- Running the algorithm on the sample ``x`` gives:
...
**The estimate obtained with just 10 samples is remarkable!**



## {.smaller}

**Is this a miracle? Sort of**

- The variance of the random sample ``x`` is

```r
var(x)
```
```{r}
set.seed(21)    # Set seed to always get same randomization
...
- Take ``var(x)`` as *empirical estimate* of the population variance $\sigma^2$, we get
$$
\Var[\overline{X}] = \frac{\sigma^2}{n} \approx 
\frac{1.504024}{10} \approx 0.15
$$


| Quantity             | True value  | Empirical est. | Bootstrap est. |
|:------------         | :-----------|:------------       |:------------       |
| $\Var[\overline{X}]$ | 0.1         | 0.15               | 0.134              |


**The bootstrap variance estimates improves on the emprical estimate!**
...
- As the sample size increases, we have that

$$
\lim_{n \to \infty} \Var[\overline{X}]_{\rm Boot} = 
\Var[\overline{X}]
$$

- Running the algorithm with a sample size of $n = 10$ gives
...
- Running the algorithm with a sample size of $n = 200$ gives
