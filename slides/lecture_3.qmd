...
# Lecture 3: <br>The t-test and <br>an introduction to R {background-color="#cc0164" visibility="uncounted"}
...
# Part 2: <br>The t-test by hand {background-color="#cc0164" visibility="uncounted"}
...
- **Note:** For the one-sided t-test, the p-value is
$$
p = 
\begin{cases}
P(t_{n-1} < t | \mu = \mu_0)  & \quad \text{ if alternative hypothesis is } \, H_1 \colon \mu < \mu_0 \\
P(t_{n-1} > t | \mu = \mu_0)  & \quad \text{ if alternative hypothesis is } \, H_1 \colon \mu > \mu_0 \\
\end{cases}
$$
...
4. ``alternative = "less" `` tells R to perform one-sided t-test
$$
...
This is a one-sided t-test. The p-value is computed as
$$
p = P( t_{n-1} < t  \, | \, \mu = 17 )  
$$
